...
(function () {
  ...
  var resolve = function (result) {
    if (!resolver) {
      console.error('resolve was called, but there is no pending Promise. (code: ' + result + ')');
      return;
    }
    ...
    if (zmusicPlaying) {
      ...
      if (zmusicPrerenderBufferReady && zmusicNode) {
        ...
        zmusicNode.port.postMessage({
          ...
          bufferR: Array.from(zmusicPrerenderBuffer[1])
        });
      }
    }
    setTimeout(updateAudioWorklet, 0);
  }
  ...
  var zmusicReady = function (code) {
    ...
    if (audioContext.audioWorklet) {
      audioContext.audioWorklet.addModule('dist/zmusic-processor.js').then(() => {
        zmusicNode = new AudioWorkletNode(audioContext, 'zmusic-processor');
        zmusicNode.connect(audioContext.destination);
  ...
  ZMUSIC = {
    ...
    ACTIVE: "active",      // started and autostart was true or play() is called,
                           // or stop() is called after playing
     ...
     * from an event handler for user initiated interactions.
     */
    resume: function () {
      if (audioContext)
        audioContext.resume();
    ...
    /**
     * Plays ZMD data with ZPD. If ZPD isn't specified, previous data will be reused.
     ...
     * @param {ArrayBuffer} data Data to compile and play.
     * @return {Promise<number>} promise
     */
     ...
     * @param {number} d1 d1 register value
     * @param {number} d2 d2 register value
     * @param {number} d3 d3 register value
     * @param {number} d4 d4 register value
     * @param {number} a1 a1 register value, could be in [0x100000:0x1FFFFF]
     * @param {ArrayBuffer} data Data that should be stored at a1 address
     * @param {number} [offset=0] offset of data
     * @param {number} [length] length of data from offset (default: data.byteLength)
     */
    trap: function (d1, d2, d3, d4, a1, data, offset, length) {
      if (a1 && zmusicBufferStart <= a1 && a1 <= zmusicBufferEnd && data) {
        ...
        for (var i = 0; i < length; ++i) {
          Module.HEAPU8[zmusicBuffer + a1 - zmusicBufferStart + i] = d8[offset + i];
        }
      }
